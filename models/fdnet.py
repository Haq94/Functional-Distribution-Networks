...
class FDNLayer(nn.Module):
    def __init__(self, condition_dim, weight_in_dim, weight_out_dim, hyper_hidden_dim):
        super().__init__()
        self.condition_dim = condition_dim
        self.input_dim = weight_in_dim
        self.output_dim = weight_out_dim
        self.weight_param_dim = self.input_dim * self.output_dim
        self.bias_param_dim = self.output_dim

        total_output_dim = 2 * (self.weight_param_dim + self.bias_param_dim)

        self.hypernet = nn.Sequential(
            nn.Linear(condition_dim, hyper_hidden_dim),
            nn.ReLU(),
